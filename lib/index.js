...
export default class Lyricist {
  ...
  */
  ...
  async getSong(id, opts) {
    if (!id) throw new Error('No ID was provided to getSong()');
  ...
  */
  ...
  async getAlbum(id, opts) {
    if (!id) throw new Error('No ID was provided to getAlbum()');
  ...
  async getArtist(id, opts) {
    if (!id) throw new Error('No ID was provided to getArtist()');
    ...
    let songs = null;
    if (opts && opts.fetchSongs) {
      const songResponse = await this._request({ path: `artists/${id}/songs` });
      songs = Array.from(songResponse.songs);
    }
    return Object.assign({ songs }, artist);
  }

  /* Get artist songs */

  async getArtistSongs(id, opts) {
    if (!id) throw new Error('No ID was provided to getArtistSongs()');
